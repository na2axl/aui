...
#define GET_Y_LPARAM(lp)    ((int)(short)HIWORD(lp))
    ...
    {
        case WM_CREATE: {
            // Extend the DWM frame into the top of the client area so DWM treats that
            // strip as caption space. Required for Win11 snap-layout flyouts on hover of
            // the HTMAXBUTTON region; a 1px margin (the old value) is too narrow for DWM
            // to recognise as a real caption.
    ...
    case WM_NCLBUTTONUP: {
        // DefWindowProc's NC → SC_* translation is unreliable when DwmDefWindowProc is not
        // participating (we don't extend the DWM frame wide enough for that). Dispatch the
        // action ourselves to guarantee minimize/maximize/close all fire.
